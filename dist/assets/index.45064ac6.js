...
function jl(t2, ...e) {
  ...
  let c, a, h;
  if (t2 instanceof ya)
    a = fa(t2.firestore, ih), h = ln(t2._key.path), c = {
      next: (n2) => {
        e[o] && e[o](Hl(a, t2, n2));
      },
      error: e[o + 1],
    ...
    const n2 = fa(t2, pa);
    a = fa(n2.firestore, ih), h = n2._query;
    const s2 = new Ml(a);
    c = {
      next: (t3) => {
  ...
  return function(t3, e2, n2, s2) {
    const i2 = new va(s2), r2 = new _c(e2, i2, n2);
    return t3.asyncQueue.enqueueAndForget(async () => ac(await Ga(t3), r2)), () => {
      i2.bc(), t3.asyncQueue.enqueueAndForget(async () => hc(await Ga(t3), r2));
    };
  }(uh(a), h, u, c);
}
function zl(t2, e) {
...
const userData = async (userId, userEmail) => {
  try {
    const docRef = await Ql(Ta(db, "users"), {
      id: userId,
      username: userEmail
    });
...
const googleUsers = async () => {
  ...
  if (user !== null) {
    const docRef = await Ql(Ta(db, "googleUsers"), {
...
const onGetPost = async (callback) => {
  const getPost = await jl(Ta(db, "posts"), callback, {});
  return getPost;
};
const postEdit = async (id2, title, description) => {
  const postRef = Aa(db, "posts", id2);
  console.log(postRef);
  await Kl(postRef, {
    title,
    description
...
const deletePost = async (id2) => {
  const eliminarPost = await Gl(Aa(db, "posts", id2));
  return eliminarPost;
};
const removeLike = async (userId, postId) => {
  const postRef = Aa(db, "posts", postId);
  const postSnapshot = await Ol(postRef);
...
const loginErrorHandler = (error) => {
  ...
  if (errorCode === "auth/invalid-email") {
    ...
    <span>Ingresa un correo v\xE1lido.</span>`;
  } else if (errorCode === "auth/user-not-found") {
    errorFeedback.innerHTML = `<img src=${errorIcon} alt="error" class="error-icon" />
    <span>Usuario no registrado.</span>`;
  } else if (errorCode === "auth/internal-error") {
    errorFeedback.innerHTML = `<img src=${errorIcon} alt="error" class="error-icon" />
    <span>Ingresa una constrase\xF1a.</span>`;
  } else if (errorCode === "auth/wrong-password") {
    errorFeedback.innerHTML = `<img src=${errorIcon} alt="error" class="error-icon" />
    <span>Correo y/o contrase\xF1a inv\xE1lidos.</span>`;
  } else if (errorCode === "auth/too-many-requests") {
    errorFeedback.innerHTML = `<img src=${errorIcon} alt="error" class="error-icon" />
    <span>Se han realizado demasiados intentos f\xE1llidos.</span>`;
  }
};
...
const login = () => {
  ...
  <form id="login-form">
    ...
    <div class="login-span">
      <span
        >\xBFTodav\xEDa no tienes cuenta? <a href="#/signup" class="span-btn">Reg\xEDstrate aqu\xED.</a
        ></span>
        </div>
  </div>
  </section>
...
const validateEmail = (email) => {
  ...
  <span>El correo ingresado no es v\xE1lido.</span>`;
  return false;
};
const validatePassword = (password) => {
  ...
  if (password.length === 0) {
    ...
    <span>Este campo no puede estar vac\xEDo.</span>`;
    return false;
  }
  if (password.length < 6) {
    passwordErrorText.innerHTML = `<img src=${errorIcon} alt="error" class="error-icon" />
    <span>La contrase\xF1a debe tener un m\xEDnimo de 6 caracteres.</span>`;
    return false;
  }
  if (password.length > 16) {
    passwordErrorText.innerHTML = `<img src=${errorIcon} alt="error" class="error-icon" />
    <span>La contrase\xF1a debe tener un m\xE1ximo de 16 caracteres.</span>`;
    return false;
  }
  return true;
};
const validateConfirmPassword = (password, passwordConfirm) => {
  ...
  if (passwordConfirm !== password) {
    passwordConfirmErrorText.innerHTML = `<img src=${errorIcon} alt="error" class="error-icon" />
    <span>Las contrase\xF1as no coinciden.</span>`;
    return false;
  }
  return true;
};
const validateNickname = (nickname) => {
  ...
  if (nickname.length > 8) {
    nicknameErrorText.innerHTML = `img src=${errorIcon} alt="error" class="error-icon" />
    <span>Debe tener m\xE1ximo 8 caracteres.</span>`;
    return false;
  }
  return true;
};
const signupErrorHandler = (error) => {
  ...
  if (errorCode === "auth/email-already-in-use") {
    alert("Este correo ya est\xE1 en uso.");
  }
};
const signup = () => {
  ...
  <section>
    <div class="container">
       ...
       <form id="signup-form">
          ...
          <div class="container-item">
            <label class="label-signup" for="signup-password">Contrase\xF1a</label>
          ...
          <div class="container-item">
            <label class="label-signup" for="signup-confirm-password">Confirmar contrase\xF1a</label>
        ...
        <div class="signup-span">
          <span>\xBFYa tienes cuenta? <a href="#/login" class="span-btn">Ingresa aqu\xED.</a></span>
        </div>
    </div>
  </section>
</main>
`;
  ...
  viewSignUp.querySelector("#signup-submit-button").addEventListener("click", async (e) => {
    ...
    try {
      ...
      alert("Se env\xEDo un enlace de verificaci\xF3n a tu correo.");
      window.location.hash = "#/login";
      return true;
    } catch (err) {
...
<main>
    ...
    <article>
      <div>
        ...
        <form class="post-form" id="post-form">
            ...
            <div>
            <textarea id="post-description" placeholder="Rese\xF1a"></textarea> 
            </div>
  ...
  onGetPost((querySnapshot) => {
    ...
    btnDelete.forEach((button) => {
      button.addEventListener("click", ({ target: { dataset } }) => {
        const isConfirmed = confirm("\xBFEst\xE1 seguro de que desea eliminar esta publicaci\xF3n?");
        if (isConfirmed) {
  ...
  dashboardPost.addEventListener("click", (e) => {
    ...
    if (title === "" || description === "") {
      ...
      const isConfirmed = confirm("\xBFEst\xE1 seguro de que desea guardar los cambios en esta publicaci\xF3n?");
      if (isConfirmed) {
...
const router = (hash) => {
  ...
  switch (hash) {
    ...
    case "#/dashboard":
      ...
      break;
    default:
      containerRoot.innerHTML = "P\xE1gina no encontrada";
  }
};
const init = () => {
  window.location.hash = "#/login";
